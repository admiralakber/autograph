...
- **An honest negative result, kept.** Autograph tested whether a *learned* "mirror brain" could invert render→DNA across the population: it does not generalise (held-out R² ≈ 0). There is **no universal mirror** — the inverse is each creature's own — and fully iterating the loop drifts to the only perfect fixed point, the empty one. *Life is imperfect self-knowledge.* That finding is part of the contribution, not hidden.
