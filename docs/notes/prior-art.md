...
- **The loop is rendered, three-dimensional and watchable.** Not "a network that prints its weights as numbers", but a creature that **draws** itself in 3-D colour and then **reads that picture back through its own brain** to reconstruct its DNA — with **loop skill (R² above the mean) measured live, never faked**.
- **Through the picture, not around it.** The decode half feeds the rendered picture back through the creature's own CPPN-painted brain — the phenotype is genuinely in the read path, and the reader is the creature's own evolved network (a continuous cousin of the [neural-network quine](https://arxiv.org/abs/1803.05859)). Two honest negative results drove this: a separate per-creature regressor collapsed to "predict the mean" (~0.97 while reconstructing nothing), and a CPPN echoing its genes at abstract coordinates *bypassed the picture*. The shipped loop fixes both; baseline-corrected skill makes the mean worth 0, a genuinely self-consistent creature scores higher (compact creatures close it tightly, richer genomes face a harder loop), and fully iterating drifts to the only effortless fixed point, the empty one. *Life is imperfect self-knowledge.*
