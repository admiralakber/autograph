...
- **NEAT** — [Stanley & Miikkulainen, 2002](https://nn.cs.utexas.edu/downloads/papers/stanley.ec02.pdf) — evolves both *weights and topology*, starting minimal and **complexifying** through add-node / add-connection with **innovation numbers** that align genes for crossover, while **speciation** protects new structure. Autograph's DNA is genuine NEAT: it begins as a minimal graph and augments its own topology on screen.
...
- **The loop is rendered, three-dimensional and watchable.** Not "a network that prints its weights as numbers", but a creature whose DNA **paints an image** of itself in 3-D colour, a brain that **emerges within** it, and which then **reads that image back through its own brain** to reconstruct its DNA — with **loop skill (R² above the mean) measured live, never faked**.
- **Through the image, not around it.** The decode half feeds the rendered image back through the creature's own CPPN-painted brain — the phenotype is genuinely in the read path, and the reader is the creature's own evolved network (a continuous cousin of the [neural-network quine](https://arxiv.org/abs/1803.05859)). Two honest negative results drove this: a separate per-creature regressor collapsed to "predict the mean" (~0.97 while reconstructing nothing), and a CPPN echoing its genes at abstract coordinates *bypassed the image*. The shipped loop fixes both; baseline-corrected, complexity-weighted skill (read through a bounded per-gene view) makes the mean worth 0 and never lets a compact creature win for free, and fully iterating drifts to the only effortless fixed point, the empty one. *Life is imperfect self-knowledge.*
