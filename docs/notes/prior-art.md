...
- **The loop is rendered, three-dimensional and watchable.** Not "a network that prints its weights as numbers", but a creature that **draws** itself in 3-D colour, with the read-back arc shown turning back into the DNA, and **loop fidelity measured live, never faked**.
- **No universal mirror — so each creature reads itself.** A single *shared* "mirror brain" inverting render→DNA across the population does not generalise (held-out R² ≈ 0). So the read-back is a genuine network but **per-creature**: each creature carries and co-evolves its own reader, learning its own inverse. And fully iterating the loop drifts to the only perfect fixed point, the empty one. *Life is imperfect self-knowledge.* The honest negative result (no universal mirror) is part of the contribution, not hidden.
