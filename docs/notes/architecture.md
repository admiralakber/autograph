...
| **Read → write loop + honest skill (v7)** — the brain reads its image (attention-chosen glimpses → recurrent ponder with Hebbian plasticity + neuromodulation → ACT halt), then **autoregressively WRITES its DNA** from its own neurons, one gene at a time, feeding back its previous output and **deciding its own length** (no quine, no length given); skill = R² above the mean (`1−MSE/Var`) on **length + values**, complexity-weighted: blank/trivial **0.000**, length-discovery genuinely evolves (emitted length → genome length, Λ≈0.9), ≈22% solo; **vitality gate** against the trivial zero-quine. Measured limit: it writes its own length yet recovers only *part* of itself | ✅ **Real, on device** | `readback.ts`, `substrate.ts`, `fitness.ts` |
...
| **Temporal brain + self-writer (v6→v7)** — the substrate runs over *T* steps (recurrence alive), CPPN-painted Hebbian plasticity + [Backpropamine](https://arxiv.org/abs/2002.10585)-style neuromodulation, [RAM](https://arxiv.org/abs/1406.6247)-style evolved glimpse attention, an [ACT](https://arxiv.org/abs/1603.08983) halt on the read, and (v7) an **autoregressive writer** — emitVal/emitEnd readouts that emit the DNA one gene at a time with a learned end-signal ([seq2seq](https://arxiv.org/abs/1409.3215)) — all *evolved* and *intrinsic*, gentle on-ramps | ✅ **Real, on device** | [v7 notes](v7-self-writer.md), `substrate.ts` |
| **Future** — a *deeper self-write* (reconstruct more than today's ~quarter of the DNA); a single dedicated *evolution Web Worker* + WASM/SIMD (a multi-core pool is precluded — it would change the numbers) | 🔭 **Roadmap** | [v7 notes](v7-self-writer.md) |
