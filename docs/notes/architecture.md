...
| **Temporal brain (next chapter)** — run the substrate over *T* steps (recurrence alive), CPPN-painted Hebbian / neuromodulated plasticity (adaptive HyperNEAT, Risi & Stanley 2010), and a read→ponder→emit loop with a learned halting gate (ACT, Graves) | 🔭 **Roadmap** *(designed, not built; needs a genome change → `ARCHIVE_EPOCH` auto-rotates; closure stays earned via the bounded read + a ponder cost)* | [whitepaper §6](../WHITEPAPER.md) |
