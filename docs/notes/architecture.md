...
| **Read → ponder → emit loop + honest skill (v6)** — the brain reads its image over a *temporal lifetime*: attention-chosen glimpses → recurrent ponder (Hebbian plasticity + neuromodulation) → ACT halt → emit DNA′; skill = R² above the mean (`1−MSE/Var`), complexity-weighted: blank/trivial **0.000**, attention load-bearing, ≈44% solo; **vitality gate** against the trivial zero-quine. Measured limit: recovers the visible *form*, not the temporal *interior* | ✅ **Real, on device** | `readback.ts`, `substrate.ts`, `fitness.ts` |
...
| **Temporal brain (v6)** — the substrate runs over *T* steps (recurrence alive), CPPN-painted Hebbian plasticity + [Backpropamine](https://arxiv.org/abs/2002.10585)-style neuromodulation, [RAM](https://arxiv.org/abs/1406.6247)-style evolved glimpse attention, and a read→ponder→emit loop with an [ACT](https://arxiv.org/abs/1603.08983) halt — all *evolved* and *intrinsic*, gentle on-ramps | ✅ **Real, on device** | [v6 notes](v6-temporal-brain.md), `substrate.ts` |
| **Future** — single dedicated *evolution Web Worker* + WASM/SIMD (a multi-core pool is precluded — it would change the numbers); *Phase 5b* dynamics-readout (reconstruct the temporal interior from the read's trace) | 🔭 **Roadmap** | [v6 notes](v6-temporal-brain.md) |
