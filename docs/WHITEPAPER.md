...
We describe **Autograph**, a browser-native instrument for a live evolutionary experiment in which small neural networks evolve toward **self-reference**. Each individual is a single **connective [CPPN](#references)** (the genotype, or "DNA") read two ways. Given a pair of 3-D coordinates it emits `[weight, bias]`; from that weight pattern a genuine **[ES-HyperNEAT](#references)** quadtree (Risi & Stanley 2012) — variance-based division/initialization and band-pruning extraction — *grows* the phenotype "brain", deciding the **placement, density and connectivity** of its hidden neurons (not a fixed grid). Queried over space, that brain outputs a density-and-hue field, rendered as a volumetric 3-D **self-portrait**. The loop's decode half is **intrinsic**: the *same* CPPN, read at its genome's own canonical coordinates, outputs its genes (DNA′) — a genuine [neural-network quine](#references) (Chang & Lipson 2018), *not* a separate read-back regressor. One function, two readings; closure is $\text{DNA}' \approx \text{DNA}$. We report it as baseline-corrected **skill** ($R^2 = 1 - \text{MSE}/\text{Var}$): a constant or random creature scores **~0** (predicting the mean earns nothing — fixing an earlier metric that read ~0.97 for flat creatures), and evolved creatures reach genuine self-consistency around **0.7–0.9**. We frame self-reference, self-replication and (prospectively) cryptographic self-commitment as instances of a single fixed-point construction, the same diagonal trick underlying Gödel's incompleteness and Kleene's recursion theorem. The population is illuminated by **MAP-Elites** quality-diversity (a grid keyed by structural complexity × mirror symmetry); because the trivial near-empty creature is a degenerate fixed point (zero variance to reconstruct, zero vitality), a **vitality gate** keeps self-reference load-bearing. The instrument renders on-device (Three.js with a Canvas 2D fallback) and **joins a live shared swarm by default**: a PartyServer-on-Cloudflare coordinator holds one shared archive, with a live peer count, a collective generation rate, and best-per-niche migration between machines (an offline mode is one click away). We are explicit about the one honest approximation in the ES-HyperNEAT — a browser-bounded quadtree depth and a 2-D placement sheet with a 3-D swept render (heterogeneous activations + CPPN-painted biases are named extensions) — and about where exactness is impossible (cross-device floating-point non-determinism). We are scrupulous about the maturity of the two technical pillars: a signed, content-addressed Merkle-DAG lineage that is **built and real today** (and persisted across sessions in IndexedDB), a zkML "proof of becoming" named honestly as a **research north star**, and a quantum framing kept strictly as **metaphor and lineage, never mechanism**.

---

## Key findings (what the live instrument actually shows)

1. **The self-encoding loop is a genuine, intrinsic neural quine.** A creature's DNA grows a brain that draws a self-portrait; the *same* CPPN, read at its genome's own coordinates, outputs its genes (DNA′) — there is **no separate read-back network**. We report **skill** as baseline-corrected $R^2$ (`1 − MSE/Var`), *measured live, never faked*: a constant or random creature scores **~0** (an earlier free-regressor metric inflated this to ~0.97 by predicting the mean — the hack this design removes), and evolved creatures reach genuine self-consistency **~0.7–0.9** (§3.1).
2. **Honesty by construction, not by audit.** Because the decode is the creature reading *itself* (nothing external to fit), the only way to score is to actually be self-consistent; "predict the mean" earns 0. And *fully iterating* the self-map drives any creature toward the only effortless fixed point — a flat creature whose genes lose all variance (vitality → 0; measured: 1.00 → 0.17 over 30 steps). So **life is imperfect self-knowledge**: a living creature can only ever *approach* closure (§3.1).
...
- a **self-quine** map $Q:\mathcal{G}\to\mathcal{G}$ — the *same* CPPN read at canonical genome coordinates: for each gene (a connection $f\!\to\!t$, or a node $n$) it queries the CPPN at that gene's address (`weight` at $(\text{coord}(f),\text{coord}(t))$; `bias` at $(\text{coord}(n),\text{coord}(n))$) to output the reconstructed gene DNA′. There is **no separate network** — nothing external to the genome.

The target is a genome $g^\star$ that is a **fixed point of the self-quine $Q$**:

$$ Q(g^\star) \approx g^\star . $$

This is exactly Chang & Lipson's neural-network quine — a network whose output, read at coordinate indices, is its own parameters [12] — realised on the connective-CPPN substrate. The self-portrait $R(B(g))$ is a *parallel* reading of the same function: one CPPN, queried over space, paints the brain; queried at its genome coordinates, reports its recipe. We do **not** assume exact fixed points are reachable (and §3.5 explains why bitwise exactness is impossible across heterogeneous hardware anyway), so the operational quantity is baseline-corrected **skill** $\in[0,1]$:

$$ \text{skill} = \max\!\Big(0,\; 1 - \tfrac{\text{MSE}(Q(g),\,g)}{\operatorname{Var}(g)}\Big), $$

the coefficient of determination of the read-back against the creature's own gene distribution. It is **measured and displayed live, never faked**, and it is honest *by construction*: predicting the per-creature mean gives skill 0, so a constant or trivial creature scores ~0 (not the ~0.97 an earlier $1-\text{RMSE}$ metric printed for flat creatures); only genuine structure-reconstruction scores above 0.

**The degenerate fixed point, and the vitality gate.** A blank, near-flat creature has *nothing to reconstruct* — its genes carry ~no variance, so its skill is ~0 — and it is also volumetrically empty. We therefore never reward closure alone: a **vitality** term (volumetric contrast, ≈0 for empty creatures) gates fitness, and the **quality-diversity** pressure (§3.3) preserves the *space* of lively self-encoders. Self-reference is only interesting when it is load-bearing against a world [12].

**Why intrinsic, and not a free regressor.** An earlier design carried a separate per-creature read-back MLP that took the rendered portrait and regressed each DNA value. It *collapsed to predicting the mean*: the DNA parameters cluster near the centre of their range, so a near-constant output (a flat grey read-back) scored ~0.97 under $1-\text{RMSE}$ while reconstructing nothing — a measurement artefact, not self-knowledge. Removing it for the **intrinsic quine** fixes the root cause: there is no free head to over-fit, and the baseline-corrected skill makes the mean worth nothing. **Measured (headless):** random and constant creatures score ~0.00; evolution lifts lively creatures to genuine self-consistency ≈ 0.7–0.9. The deeper truth is preserved: **fully iterating** $Q$ drives any creature toward the *only* effortless fixed point — a flat creature whose genes lose all variance (vitality → 0; measured drift 1.00 → ≈0.17 over 30 steps). Perfect, effortless self-encoding is emptiness; a living creature only ever *approaches* closure. So we score one-step skill and let the vitality gate + quality-diversity hold the population lively-but-imperfect. **Life is imperfect self-knowledge.**
...
| The loop | relaxation α / tolerance | 0.55 / 0.012 | the iterate-to-fixed-point map $g \leftarrow g + \alpha(Q(g)-g)$ (Q = self-quine) |
