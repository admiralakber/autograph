...
We describe **Autograph**, a browser-native instrument for a live evolutionary experiment in which small neural networks evolve toward **self-reference**. Each individual is a single **connective [CPPN](#references)** (the genotype, or "DNA") read two ways. Given a pair of 3-D coordinates it emits `[weight, bias]`; from that weight pattern a genuine **[ES-HyperNEAT](#references)** quadtree (Risi & Stanley 2012) — variance-based division/initialization and band-pruning extraction — *grows* the phenotype "brain", deciding the **placement, density and connectivity** of its hidden neurons (not a fixed grid). Queried over space, that brain outputs a density-and-hue field, rendered as a volumetric 3-D **image** — the image the creature is born in. The loop closes **through the image**: a brain *emerges within* that image, then reads back the image it's born in through its *own* hidden neurons, which outputs DNA′ — a reconstruction of its connection vector (it tries to *find its own beginning*). The reader is the creature's own evolved network — the image is genuinely in the read path (it is *not* a separate regressor, and *not* the CPPN merely echoing itself at abstract coordinates) — a continuous cousin of the [neural-network quine](#references) (Chang & Lipson 2018). Closure is $\text{DNA}' \approx \text{DNA}$, reported as baseline-corrected **skill** ($R^2 = 1 - \text{MSE}/\text{Var}$), **complexity-weighted** and read through a deliberately **bounded, per-gene view** of the image: a blank or trivial image scores **~0** (predicting the mean earns nothing — fixing an earlier metric that read ~0.97 for flat creatures), a *compact* creature **no longer closes for free** (the old ~0.93 drops substantially), and closing *more* of yourself counts for more — a humbling, earned headline that climbs but is never "solved". We frame self-reference, self-replication and (prospectively) cryptographic self-commitment as instances of a single fixed-point construction, the same diagonal trick underlying Gödel's incompleteness and Kleene's recursion theorem. The population is illuminated by **MAP-Elites** quality-diversity (a grid keyed by structural complexity × mirror symmetry); because the trivial near-empty creature is a degenerate fixed point (zero variance to reconstruct, zero vitality), a **vitality gate** keeps self-reference load-bearing. The instrument renders on-device (Three.js with a Canvas 2D fallback) and **joins a live shared swarm by default**: a PartyServer-on-Cloudflare coordinator holds one shared archive, with a live peer count, a collective generation rate, and best-per-niche migration between machines (an offline mode is one click away). We are explicit about the one honest approximation in the ES-HyperNEAT — a browser-bounded quadtree depth and a 2-D placement sheet with a 3-D swept render (heterogeneous activations + CPPN-painted biases are named extensions) — and about where exactness is impossible (cross-device floating-point non-determinism). We are scrupulous about the maturity of the two technical pillars: a signed, content-addressed Merkle-DAG lineage that is **built and real today** (and persisted across sessions in IndexedDB), a zkML "proof of becoming" named honestly as a **research north star**, and a quantum framing kept strictly as **metaphor and lineage, never mechanism**.

---

## Key findings (what the live instrument actually shows)

1. **The self-encoding loop runs through the image.** A creature's DNA paints an image; a brain *emerges within* it; the brain then **reads back the image it's born in through its own hidden neurons** to output DNA′ — the phenotype is genuinely in the read path, and the reader is the creature's own evolved network (no separate regressor). We report **skill** as baseline-corrected $R^2$ (`1 − MSE/Var`), *complexity-weighted and read through a bounded, per-gene view*, *measured live, never faked*: a blank/trivial image scores **~0** (an earlier free-regressor metric inflated this to ~0.97 by predicting the mean — the hack this design removes), a *compact* creature no longer wins for free (the old ~0.93 drops substantially), and closing *more* of yourself counts for more — a humbling, earned headline that climbs but is never "solved" (§3.1).
2. **Honesty by construction, not by audit.** Because the decode reads the creature's *own image* through its *own brain* (nothing external to fit) — and only a bounded, per-gene view of it — the only way to score is genuine self-consistency; a blank image / "predict the mean" earns 0. And *fully iterating* the loop drives any creature toward the only effortless fixed point — a flat creature whose genes lose all variance (vitality → 0; measured: 1.00 → ≈0.17 over 30 steps). So **life is imperfect self-knowledge**: a living creature can only ever *approach* closure (§3.1).
...
- a **read-back** map $E:\mathcal{X}\to\mathcal{G}$ — the rendered image, fed *back through the creature's own brain*: the density field is sampled at $F$ fixed probes, those samples drive the substrate's own hidden neurons (input→hidden weights painted by the same CPPN at the probe/neuron coordinates), and each gene is read off the hidden layer at its canonical genome coordinate (hidden→gene weights, again CPPN-painted) → DNA′. The reader is the creature's **own** evolved network; there is **no separate regressor**, and crucially the **image is in the path** (the CPPN is not merely echoing itself at abstract coordinates).
...
the coefficient of determination of the read-back against the creature's own gene distribution. It is **measured and displayed live, never faked**, and honest *by construction*: a blank image drives the brain to a near-constant DNA′ ≈ the per-creature mean → skill 0 (not the ~0.97 an earlier $1-\text{RMSE}$ free-regressor metric printed for flat creatures); only an image that genuinely carries its DNA, read back by its own brain, scores above 0.
...
**Honest skill, and what it depends on.** An earlier design carried a separate per-creature read-back MLP that regressed each DNA value from the rendered image's fingerprint; it *collapsed to predicting the mean* (a flat-grey output scored ~0.97 under $1-\text{RMSE}$ while reconstructing nothing — a measurement artefact). A second design had the CPPN echo its genes at abstract coordinates — honest as an $R^2$, but it *bypassed the image* (the phenotype was not in the read path). The shipped loop fixes both: the **image is the input** and the **creature's own brain is the reader**. The skill is then **hardened** two ways so closure is genuinely earned: the brain gets only a **bounded, per-gene view** of its image (a few probes per gene, floored and capped — so reconstruction is honestly hard *at every scale*), and the $R^2$ is **weighted by genome complexity** (closing *more* of yourself counts for more). Random and constant creatures score ≈ 0.00; a *compact* creature no longer closes for free — an earlier, looser read let live compact champions sit at ≈ 0.9, which this hardening deliberately removes. **Two honest numbers, and the gap between them is itself a result.** *Solo*, a single garden is humbled: in headless runs the honest headline climbs from 0 to only **≈ 0.25** over a couple of thousand generations, and keeps climbing — the loop is genuinely hard for one machine alone. *Collectively*, the live swarm (many machines + best-per-niche migration, currently on `genesis-v5`) reaches far higher: we have observed a richer **≈ 24-gene** self-encoder close its loop at **≈ 0.92**, genuinely earned — **the crowd discovers what one mind alone cannot.** This is *not* the old compact freebie: a blank or random creature still scores ≈ 0, a compact creature is capped by the bounded per-gene read, and complexity is rewarded (closing *more* of yourself counts for more). The number is whatever it honestly is — never faked, and a blank creature never inflates it. The deeper truth is preserved: **fully iterating** $E\circ R\circ B$ drives any creature toward the *only* effortless fixed point — a flat creature whose genes lose all variance (vitality → 0; measured drift 1.00 → ≈0.17 over 30 steps). Perfect, effortless self-encoding is emptiness; a living creature only ever *approaches* closure. **Life is imperfect self-knowledge.**
...
| Substrate (ES-HyperNEAT) | initial / max quad depth | 1 / 2 | minimum sampling resolution / **browser-bounded max resolution** ($r_m$) |
...
| Substrate (ES-HyperNEAT) | iteration level / max hidden | 1 / 32 | hidden→hidden discovery passes / defensive neuron cap |
...
| The loop | read-back probes $F$ | 24 | image samples fed back through the brain (read-back resolution) |
...
- **The temporal brain (the next chapter).** Today the brain is essentially single-shot: the draw pass settles in a fixed couple of passes and the read-back is one feed-forward sweep (§3.1–3.2). The next direction makes it a *dynamical* processor, in dependency order: (i) run the substrate for $T$ steps so the **recurrence** it already evolves does real work; (ii) have the CPPN paint a per-connection **plasticity rule** $[\eta, A, B, C, D]$ — *adaptive HyperNEAT* [27, 28] — with gates acting as **neuromodulators** [28], so a creature learns toward self-knowledge *within its lifetime*; and (iii) ingest the image as a sequence and **read → ponder → emit** DNA′ autoregressively under a learned **halting gate** with a ponder cost (Adaptive Computation Time; RAM-style learned glimpses [29]). The wager is that open-ended, crowd-scale QD is exactly the engine to discover the brittle halting/plasticity policies that hand-design and gradient descent find hard — a million browsers are a million shots, and MAP-Elites banks the rare wins. **The honesty guardrails carry over:** the bounded per-gene read, the complexity-weighted skill, and an explicit ponder cost keep closure *earned* (ponder-until-ready must not re-trivialise it); a blank/trivial creature still scores ≈ 0, measured live. It implies a genome change (plastic + halting/recurrent CPPN outputs) — painless now that `ARCHIVE_EPOCH` auto-rotates the shared world. **Designed, not built.**
...
28. S. Risi & K. O. Stanley. *Indirectly Encoding Neural Plasticity as a Pattern of Local Rules* (adaptive HyperNEAT). SAB 2010, LNCS 6226:533–543. https://link.springer.com/chapter/10.1007/978-3-642-15193-4_50 · review: A. Soltoggio, K. O. Stanley & S. Risi, *Born to Learn: the Inspiration, Progress, and Future of Evolved Plastic Artificial Neural Networks*, Neural Networks 108:48–67, 2018. https://arxiv.org/abs/1703.10371
29. A. Graves. *Adaptive Computation Time for Recurrent Neural Networks*. arXiv:1603.08983, 2016. https://arxiv.org/abs/1603.08983 · V. Mnih, N. Heess, A. Graves & K. Kavukcuoglu. *Recurrent Models of Visual Attention*. NeurIPS 2014. arXiv:1406.6247. https://arxiv.org/abs/1406.6247 · I. Sutskever, O. Vinyals & Q. V. Le. *Sequence to Sequence Learning with Neural Networks*. NeurIPS 2014. arXiv:1409.3215. https://arxiv.org/abs/1409.3215 *(refs 28–29 underpin the roadmap §6 "temporal brain", not the shipping system.)*
