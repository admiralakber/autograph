...
**The read-back, and the "mirror brain" we tested.** Today the read-back map $E$ is *analytic*: it reads the painted density at each probe and de-normalises it into the matching DNA parameter — a per-creature inverse that recovers the DNA at the loop fidelity shown live (~0.9 for evolved creatures). The natural next question is whether a *network* — a co-evolved "mirror" brain — could do the read-back instead. We ran the honest experiment: a shared encoder (linear, and a small MLP) fit from a fixed portrait fingerprint to a fixed DNA fingerprint across a whole population. **It does not generalise** — held-out $R^2 \approx 0$, barely beating the mean. The render→DNA inverse is *creature-specific*, not one shared learnable map: a portrait encodes *its own* DNA, but there is no universal mirror. And the deeper truth is invariant to the choice of $E$: **fully iterating** $E\circ R\circ B$ drives any creature toward the *only* perfect fixed point — the trivial flat zero-quine (vitality 0). Perfect self-encoding is emptiness; a living creature can only ever *approach* closure. So we score one-step fidelity and let the vitality gate + quality-diversity hold the population lively-but-imperfect. **Life is imperfect self-knowledge** — and we never fake the closure.
...
**The DNA (genotype).** A *connective* CPPN [6,25] evolved by genuine **NEAT — augmenting topologies** [1]. The genome is a *graph*: node genes (each with a heterogeneous activation drawn from `sin`, `gauss`, `tanh`, `sigmoid`, `abs`, `cos`, `relu`, `triangle`, `identity`) and connection genes carrying **innovation numbers** (historical markings). It **starts minimal** — every input wired straight to every output — and *complexifies* over generations through the structural operators **add-connection** and **add-node** (which splits an existing connection), with **recurrent links permitted** (a compiled, recurrence-aware evaluator runs a few propagation passes). Crossover aligns genes by innovation number. It still maps a pair of 3-D node positions to a `weight` and a link-expression gate (`leo`). Because the genome grows, so does the loop: the read-back map $E$ must recover **one value per parameter** (connection weight or bias), so a more complex creature faces a strictly harder self-encoding problem — complexity is not free.
