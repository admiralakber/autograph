...
**The loop is iterated, not merely scored.** Writing $T = E\circ R\circ B$ for the compose-and-read-back map, the instrument does not stop at a single self-consistency score: it runs the **fixed-point iteration** $g_{n+1} = g_n + \alpha\,(T(g_n) - g_n)$ (under-relaxation, $\alpha\approx0.55$, same fixed points as $T$ but approached gently so it can be watched) and lets the creature **settle**. Empirically an evolved self-encoder is an *attractive* fixed point — drift $\lVert g_{n+1}-g_n\rVert \to 0$ and fidelity $\to 1.000$ within a handful of steps — whereas a random creature only reaches a **residual floor** (honest *partial closure*, displayed as such, never faked). This is the [neural-network quine's regeneration / fixed-point iteration](#references) [12] made literal and visible: the loop does not merely *score* high, it *closes*, and you watch it close in the strange-loop panel.
...
flowchart LR
  ...
  X -->|"E: read density at probes"| G2["🧬 DNA′ = T(g) = E(R(B(g)))"]
  G2 -. "iterate g ← g + α·(T(g) − g): drift → 0, fidelity → 1 — watch it settle (measured live)" .-> G
```
