...
We've spent a weekend trying to make that picture *run*. Not a clever illusion of it, but an actual evolving creature — a tiny neural network — whose DNA paints a glowing image, grows a brain within it, and reads that image back to recover the very DNA it began as. A loop that closes. We call it **Autograph**, and you don't scroll it — you *join* it. It opens as a full-screen, greyscale **instrument**: mission-control for a live experiment, with the only colour reserved for the life growing inside it.
...
- The **DNA** is the genotype: a small network that, when you hand it two points in space, answers with six things — how those points should be **wired** (a connection weight and, read at a single point, a neuron's bias), the **density and colour** it paints there (its self-portrait — this is *CPPN-art*, the trick behind Picbreeder), and two **faculty** dials (plasticity and neuromodulation). We draw it as a little graph of nodes and edges.
...
Ask the **DNA**, at every point in a volume, "how much life is here, and what colour?" and it answers with a field of density and hue. Render that field and you get the creature: a volumetric cloud of points, glowing in sunrise colours. That cloud **is** the DNA, expressed — its appearance — and the brain grows from the *same* recipe, living inside the picture it will later read. In the instrument you can flip the same individual between all three faces — the render, the brain, the DNA — until the equivalence stops being a sentence and becomes obvious.

## The loop, and why it never lies 🔁

Now the strange part — and here's where we kept changing our minds until it was honest. We do **not** read the DNA back with a separate little network (we tried that; a free regressor just learns to "predict the mean" and prints a flat grey that scores ~97% while reconstructing nothing). Nor do we let the DNA simply echo itself at abstract coordinates (we tried that too — honest as a number, but it *skips the image* entirely). Instead, the loop runs **through the image** — the brain genuinely **writes itself**: the DNA paints an image, a brain grows from the same recipe, and that brain *reads* **the image it's born into** like a small act of contemplation — taking attention-chosen [glimpses](https://arxiv.org/abs/1406.6247) (it decides where to look), pondering over recurrent steps as its synapses self-modify — and then **writes its DNA autoregressively**, one gene at a time — a *real value* per step, not a discrete token — from its own **output neurons** (the writing is the brain *behaving*, never a channel painted into the DNA), feeding back its previous output ([seq2seq](https://arxiv.org/abs/1409.3215)) and **halting when it decides it's done**, so the creature itself chooses how long its DNA is. No re-projecting through the recipe, no length handed to it — the older quine entanglement is gone. The hand it drew redraws the hand. It's the continuous, rendered cousin of Chang & Lipson's [neural-network quine](https://arxiv.org/abs/1803.05859), now genuinely self-writing. And we keep it honest: writing your own DNA from scratch is hard, so the headline is humbling and earned, never "solved".

We call that match **loop skill**, measured honestly as R² above the baseline (`1 − MSE/Var`), on both the **length** and the **values**, weighted by how much of itself it closes: how much of its own DNA the brain writes back *from the image it's born in*, beyond just guessing the average. The single most important promise: *it is measured live and never faked.* A fresh, random or blank creature scores **~0** — not the old inflated ~97% — because a writer that's off emits a constant and predicting the mean earns nothing. Then comes the loveliest true thing we found: **the crowd discovers what one mind alone cannot.** A single machine, evolving solo, is humbled — the self-write is genuinely hard — its skill climbs to **roughly a fifth to a third** (≈20–30%) over a couple of thousand generations, still climbing (and you can *watch* the emitted length converge toward the genome's as it learns, length-match ≈ 0.9). The shared swarm climbs far higher: the best-per-niche migration that carried earlier worlds' self-encoders far above the lone explorer is unchanged, and the collective ceiling is the fresh world's to write. Whatever it reaches, it's the honest number, never a flattering one. And there are **two** measured ways a living creature can't perfectly know itself: *fully* iterating the loop drifts toward the only effortless fixed point — a blank creature that "draws itself" by saying nothing (vitality → 0); and even reaching for its *whole* self, the brain grasps only a fraction of its DNA. **We reach for our true selves and hold only a part.** We refuse the hollow victory with a **vitality** check and diversity pressure, so the only way to win is to be **both** alive **and** genuinely self-consistent.
