...
Now the strange part — and here's where we changed our minds about how to do it honestly. The decode half of the loop is **not** a separate little network reading pixels (we tried that; a free regressor just learns to "predict the mean" and prints a flat grey that scores ~97% while reconstructing nothing). Instead it is *intrinsic*: the **same CPPN**, read at its genome's own canonical coordinates, reports its own genes straight back. One function, two readings — over space it paints the portrait; at its genome coordinates it returns its DNA′. That's a genuine [neural-network quine](https://arxiv.org/abs/1803.05859) (Chang & Lipson), with nothing external to over-fit or game.

We call that match **loop skill**, measured honestly as R² above the baseline (`1 − MSE/Var`): how much of its own DNA it reconstructs *beyond just guessing the average*. The single most important promise: *it is measured live and never faked.* A fresh, random or constant creature scores **~0** — not the old inflated ~97% — because predicting the mean now earns nothing. As evolution does its work, lively creatures climb to genuine self-consistency around **0.7–0.9**. It never reaches a perfect 1.0 for a living creature, and that's not a bug — it's the truth of the thing: *fully* iterating the self-map drifts toward the only effortless fixed point, a blank creature that "draws itself" by saying nothing (the zero quine, vitality → 0, which we measured). We refuse that hollow victory with a **vitality** check and diversity pressure, so the only way to win is to be **both** alive **and** genuinely self-consistent.
...
So here is our move. Take Picbreeder's crowd-grown CPPNs. Take the HyperNEAT idea that a CPPN can *paint a whole other network* from geometry. Take Chang and Lipson's astonishing 2018 result, the [**neural network quine**](https://arxiv.org/abs/1803.05859) — a network that outputs its own weights, the first artificial thing that could *say* itself — which, tellingly, uses that very coordinate→weight trick. And fuse them: evolve a DNA that **both paints a self-portrait in space and, read at its own genome coordinates, reports itself**. Not "a network that prints its weights as numbers," but a creature that **draws** itself in three dimensions, in colour — and is the very function that reads itself back.
...
2. **Fork the instrument.** It's open source and dependency-light — a real evolving CPPN + genuine ES-HyperNEAT substrate, an intrinsic self-quine, a 3-D render with a plain-canvas fallback so nobody's excluded, and a Web-Crypto tree of life.
