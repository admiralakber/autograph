...
Now the strange part. Take that finished self-portrait and feed it through a second little network the creature carries — its **read-back network** — whose only job is to look at the portrait and output the DNA that drew it. Line that reconstructed DNA′ up against the creature's own numbers. The closer they match, the more the creature has managed to **draw a true picture of itself**. Crucially this reader is *part of the creature* and **co-evolves** with it: each one learns to read *itself*. (We tried a single shared "mirror" network for everyone — it doesn't generalise; the inverse is each creature's own.)
