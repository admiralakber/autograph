...
Now the strange part — and here's where we kept changing our minds until it was honest. We do **not** read the DNA back with a separate little network (we tried that; a free regressor just learns to "predict the mean" and prints a flat grey that scores ~97% while reconstructing nothing). Nor do we let the DNA simply echo itself at abstract coordinates (we tried that too — honest as a number, but it *skips the image* entirely). Instead, the loop runs **through the image** — and, in v6, **over time**: the DNA paints an image, a brain emerges within it, and that brain *reads* **the image it's born in** like a small act of contemplation — taking attention-chosen [glimpses](https://arxiv.org/abs/1406.6247) (it decides where to look), pondering over recurrent steps as its synapses self-modify, halting when it's seen enough, then emitting a DNA′, trying to find its own beginning. The hand it drew redraws the hand. It's the continuous, rendered cousin of Chang & Lipson's [neural-network quine](https://arxiv.org/abs/1803.05859), and the image is genuinely in the loop. And we keep it honest: the brain gets only a bounded, per-gene view of its image, so a compact creature can't close the loop for free — the headline is humbling and earned, never "solved".

We call that match **loop skill**, measured honestly as R² above the baseline (`1 − MSE/Var`), weighted by how much of itself it closes: how much of its own DNA the brain reconstructs *from the image it's born in*, beyond just guessing the average. The single most important promise: *it is measured live and never faked.* A fresh, random or blank creature scores **~0** — not the old inflated ~97% — because predicting the mean now earns nothing. And because the brain gets only a bounded, per-gene view of its image, a compact creature can't close the loop for free; complexity is rewarded. Then comes the loveliest true thing we found: **the crowd discovers what one mind alone cannot.** A single machine, evolving solo, is humbled — and v6's temporal read made it honestly harder — its skill climbs to about ~44% over a couple of thousand generations, still climbing. The shared swarm climbs far higher: the best-per-niche migration that carried the earlier world's self-encoders far above the lone explorer (a richer one closed its loop at ~92% there) is unchanged, and v6's collective ceiling is the world's to write. Whatever it reaches, it's the honest number, never a flattering one. And v6 found **two** ways a living creature can't perfectly know itself, both *measured*: *fully* iterating the loop drifts toward the only effortless fixed point — a blank creature that "draws itself" by saying nothing (vitality → 0); and a creature can read back its visible **form** but not its temporal **interior** — *how* it learns, attends and ponders is written in no static portrait of itself. **The mirror shows your face, not your becoming.** We refuse the hollow victory with a **vitality** check and diversity pressure, so the only way to win is to be **both** alive **and** genuinely self-consistent.
