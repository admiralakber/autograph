...
export const PARAMS: readonly ParamSpec[] = [
  ...
  { key: 'emitMaxLen', label: 'connection write cap', value: 96, group: 'Substrate (ES-HyperNEAT)', note: 'hard cap on the CONNECTION write of the structural self-write: the brain emits connection genes (from, to, weight, enabled) until its conn-end signal fires, else at this cap. Off at birth ⇒ a constant write that never halts ⇒ predict-the-mean ⇒ skill 0.' },
  { key: 'nodeMaxLen', label: 'node write cap', value: 48, group: 'Substrate (ES-HyperNEAT)', note: 'hard cap on the NODE write of the structural self-write: the brain emits node genes (activation type + bias) until its node-end signal fires, else at this cap.' },
  ...
  { key: 'lengthShapeFloor', label: 'size-shape floor', value: 0.25, group: 'The loop', note: 'the SIZE-shaping factor floor for the structural self-write. skill ×= floor + (1−floor)·Λ for each of the node-count and connection-count matches (Λ = 1−|written−actual|/actual), so getting the STRUCTURE SIZE right is load-bearing — a wrong-size graph loses most of its credit, and the creature must learn to halt at its own node/connection counts.' },
  { key: 'curriculumLo', label: 'curriculum lo (R²)', value: 0.1, group: 'The loop', note: 'below this teacher-length weight-R², a creature is graded mostly on the dense teacher read (the first G genes, free-run input, no leakage). MINIMAL scaffolding, and NECESSARY: a fully-cold fitness is gamed by degenerate writes that nail one high-variance gene — the teacher forces WHOLE-graph reconstruction, not cherry-picking.' },
  { key: 'curriculumHi', label: 'curriculum hi (R²)', value: 0.28, group: 'The loop', note: 'above this teacher-length weight-R², a creature is graded almost entirely on its OWN self-decided graph (self-length values): minimal hand-holding, so the displayed champion is the genuine self-reconstructor, not an early-halting teacher-gamer. The fast hand-over is what keeps the headline honest.' },
  ...
  { key: 'actFloor', label: 'activation-match floor', value: 0.5, group: 'The loop', note: 'structural self-write — the coupling floor for ACTIVATION-TYPE reconstruction: skill ×= actFloor + (1−actFloor)·actAccuracy. Floored so a creature with good values but chance-level activations still scores (a climbable gradient), yet getting the activation types right lifts the score toward full — coupled, but bootstrappable.' },
  { key: 'topoFloor', label: 'topology-match floor', value: 0.45, group: 'The loop', note: 'structural self-write — the coupling floor for TOPOLOGY (which node connects to which): skill ×= topoFloor + (1−topoFloor)·topoMatch. The hardest part of self-reconstruction; floored so partial/near-miss wiring earns a gradient (graded by slot proximity), tightening as the exact topology is found.' },
