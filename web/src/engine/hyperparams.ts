...
export const PARAMS: readonly ParamSpec[] = [
  ...
  { key: 'neuromodScale', label: 'neuromod scale', value: 1.0, group: 'Substrate (ES-HyperNEAT)', note: 'v6 Phase 3 — max magnitude of the per-connection neuromodulation gate g (painted by the CPPN’s 5th output, bounded by tanh×scale). The brain emits its own signal m(t)=tanh(mean of emit·activity); the gated Hebbian update is trace ← (1−η)·trace + η·(1 + g·m(t))·(pre·post), so m(t) modulates the learning rate (Backpropamine form, EVOLVED). g starts 0 (gentle on-ramp) and arises by mutation; g=0 or m=0 ⇒ exactly the Phase 2 plastic update.' },
  ...
  { key: 'lengthShapeFloor', label: 'length-shape floor', value: 0.6, group: 'The loop', note: 'v7 — the length-shaping factor floor. skill ×= floor + (1−floor)·Λ, where Λ = 1−|selfLen−G|/G rewards the creature writing the RIGHT LENGTH. Floored (never zeroed) so a good-values / wrong-length creature is still rewarded yet pressured to fix its length — a separate length gradient, no fitness valley.' },
  { key: 'curriculumLo', label: 'curriculum lo (R²)', value: 0.2, group: 'The loop', note: 'v7 length-discovery curriculum — below this teacher-length R², a creature is graded almost entirely on the DENSE teacher-length read (first G emitted values, free-run input, no leakage), which bootstraps value-generation while the end-signal is still on-ramping.' },
  { key: 'curriculumHi', label: 'curriculum hi (R²)', value: 0.55, group: 'The loop', note: 'v7 length-discovery curriculum — above this teacher-length R², a creature is graded almost entirely on its OWN self-length write (it must decide its length right to keep scoring). Between lo/hi the grading anneals smoothly (smoothstep), handing the length decision over as the creature masters values.' },
