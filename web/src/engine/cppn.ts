...
// 3-D coordinates it emits [weight, bias | α, emit, modGate, fixX, fixY, fixScale]
// (all but weight+bias are v6 TEMPORAL channels, off at birth — see arch.ts);
// ES-HyperNEAT uses the weight pattern to grow the brain (substrate.ts), and the
// SAME CPPN read at canonical genome coordinates reconstructs its own genes (quine).
//
...
export function minimalGenome(rng: Rng): Genome {
  ...
  // fixScale(14)]. ONLY the two IMAGE channels (weight, bias) are wired + biased at
  // birth; every TEMPORAL channel (α plasticity, emit, modGate, and the Phase 4
  // attention readouts — the deferred ones, fork (B)) starts with NO incoming
  ...
  // plasticity, neuromodulation AND attention, never the whole space at once.
...
export function evalCompiled(c: Compiled, x1: number, y1: number, z1: number, x2: number, y2: number, z2: number, out: number[] = [0, 0, 0, 0, 0, 0, 0, 0]): number[] {
  IN_BUF[0] = x1;
  IN_BUF[1] = y1;
  IN_BUF[2] = z1;
  IN_BUF[3] = x2;
  ...
  const oi = c.outIdx;
  for (let j = 0; j < oi.length; j++) out[j] = val[oi[j]!]!; // [weight, bias, α, emit, modGate, fixX, fixY, fixScale]
  return out;
}

/** Convenience: compile + evaluate once (not for hot loops). */
export function evalCPPN(g: Genome, x1: number, y1: number, z1: number, x2: number, y2: number, z2: number, out: number[] = [0, 0, 0, 0, 0, 0, 0, 0]): number[] {
