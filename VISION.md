...
- **Real is labelled real; illustrative is labelled illustrative.** Today the evolving DNA + substrate, the simplified ES neuron-placement, the 3-D volumetric render, the live loop-fidelity measurement, the signed lineage with cross-session persistence, and the MAP-Elites population are **real and running on your device**. The worldwide swarm, the zero-knowledge "proof of becoming", the quantum framing, and *full quadtree* ES-HyperNEAT are **directions** — and we say so, plainly, wherever they appear.
...
- **Self-reference must be load-bearing.** A blank creature "encodes itself" perfectly and means nothing — the *zero quine*. So we never reward closure alone: a **vitality gate** plus **MAP-Elites quality-diversity** keep the population pushing against a real world, exactly as a self-replicator coupled to a task must (Chang & Lipson's lesson, grounded in [`research/`](./research/)).
