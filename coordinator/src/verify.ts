...
// Mirrors web/src/engine/arch.ts. The v10 STRUCTURAL build makes the self-portrait a true
// depiction of the BUILT NETWORK (rendered from the substrate, not a CPPN channel), so the
// density/hue appearance channels are retired and CPPN_OUTPUTS drops 6→4: the full set is now
// weight, bias, α, modGate (structure + faculties). genomeBytes writes this count into the
// header, so the coordinator MUST match the engine or v10 genomes fail the integrity re-hash.
// The byte LAYOUT is unchanged from v3 — only this count.
const CPPN_INPUTS = 7;
const CPPN_OUTPUTS = 4;
...
// clean-architecture fix: CPPN_OUTPUTS 11→6 (behaviours → output neurons; density/hue → CPPN-art).
// v7 = the v10 STRUCTURAL build: byte layout STILL UNCHANGED, but CPPN_OUTPUTS drops 6→4 — the
// self-portrait is now rendered from the BUILT NETWORK (not a CPPN channel), so density/hue are
// retired (just weight, bias, α, modGate). The header's OUTPUTS field differs, so old/new genomes
// are mutually non-verifying — the epoch rotates (genesis-v10). If the engine's genome encoding
// changes again, `npm run make-fixture && npm run smoke` trips — the drift gate.
const GENOME_FORMAT_VERSION = 7;
